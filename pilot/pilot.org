...
rename their files, calculate the phylogeny, and analyze the
phylogeny.
#+end_export
...
complete genomes, we get 7,884. This tells us that only a small
fraction, $(538+22)/7884\times 100\approx 7\%$ of \emph{E. coli}
genomes are classified below the species level, when probably all
\emph{E. coli} genomes also belong to a category below the species
level.
#+end_export
#+begin_src sh <<Get genomes, Ch. \ref{ch:p}>>=
...
#+end_src
#+begin_export latex
We plot the tree to get Figure~\ref{fig:o111}, where all taxa with
...
#+end_src
#+begin_export latex
Similarly, the directory of neighbors genomes, \ty{neighbors}, holds
...
#+end_src
#+begin_export latex
This yields 157\,kb of potential marker material.
#+end_export
#+begin_src sh <<pilot.sh>>=
cres pilot.fasta
#+end_src
...
the top primer pairs from them, the best of which has the very low
penalty of $1.4\times 10^{-4}$.
#+end_export
#+begin_src sh <<pilot.sh>>=
  fa2prim pilot.fasta |
      primer3_core |
