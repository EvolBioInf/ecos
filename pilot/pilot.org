#+begin_export latex
Our working directory is \ty{tutorial} inside the \ty{o111h8} repo. We
start by linking the Neighbors database.
#+end_export
#+begin_src sh <<pilot.sh>>=
  ln -s ../data/neidb
#+end_src
#+begin_export latex
We focus on \emph{E. coli} O111:H8. So we get its genomes and that of
some of its close relatives, rename their files, calculate the
phylogeny, and analyze the phylogeny.
#+end_export
#+begin_src sh <<pilot.sh>>=
...
complete genomes, we get 7,854. This tells us that only a small
fraction, $(538+22)/7854\times 100\approx 7\%$ of \emph{E. coli}
...
\end{verbatim}
These file names later become leaf labels in our phylogeny. Since our
...
#+end_src
#+begin_export latex
We use sqlite3 to link the genome accession with its respective taxID.
#+end_export
