#+begin_export latex
Our working directory remains \ty{tutorial} inside the \ty{ecos} repo.
We focus on \emph{E. coli} serotype O111:H8, which we analyze in four
steps. We get its genomes and that of some of its close relatives,
rename the genome files, calculate the genome phylogeny, and analyze
the phylogeny.
#+end_export
...
nor to the parent, \emph{E.~coli}.  We look at the first 30 rows of
the list of complete target and neighbor genomes for O111:H8. For each
genome we get three pieces of information, a ``t'' or an ``n''
indicating target or neighbor, the accession, and the taxon-ID.
#+end_export
#+begin_src sh <<Get genomes, Ch. \ref{ch:p}>>=
...
#+end_src
#+begin_export latex
There are 538 neighbor genomes, that is, genomes classified as
...
complete genomes, we get 7,884. This tells us that only 7\,\% of
\emph{E. coli} genomes are classified below the species rank, when
probably far more \emph{E. coli} genomes also belong to known
categories below the species rank.
#+end_export
#+begin_src sh <<Get genomes, Ch. \ref{ch:p}>>=
...
neighbor genomes listed, a total of 50, extract their accessions and
store them in \ty{acc.txt}.
#+end_export
#+begin_src sh <<Get genomes, Ch. \ref{ch:p}>>=
  neighbors -l -L complete -t 991910 neidb |
      grep '^[tn]' |
...
\ty{acc.txt} in ``dehydrated'' form excluding any ``atypical'' genomes
the sample might contain.
#+end_export
#+begin_src sh <<Get genomes, Ch. \ref{ch:p}>>=
...
rather long names, for example
\begin{verbatim}
GCF_000981485.1_EcoliK12AG100_genomic.fna
\end{verbatim}
These file names later become the leaf labels in our phylogeny. Since
our aim is to classify genomes, we rename the files on the pattern
...
#+end_src
#+begin_export latex
For renaming, we run the script \ty{rename.sh}.
#+end_export
#+begin_src sh <<Rename genome files, Ch. \ref{ch:p}>>=
  bash rename.sh
#+end_src
#+begin_export latex
We check the renaming worked.
#+end_export
#+begin_src sh <<Rename genome files, Ch. \ref{ch:p}>>=
  ls all | head
#+end_src
#+begin_export latex
\begin{verbatim}
37762_GCF_001559615.2
37762_GCF_001559635.1
83333_GCA_015290945.1
83333_GCF_000800765.1
83333_GCF_000974405.1
...
\end{verbatim}
...
#+end_src
#+begin_export latex
We plot the tree to get Figure~\ref{fig:o111}, where the 22 taxa with
the taxon-ID of O111:H8, 991910, cluster together, as they
should. Moreover, the 991910 clade contains no genomes classified as
something other than O111:H8.
\begin{figure}
  \begin{center}
...
take when analyzing a tree so large that manual inspection becomes
cumbersome. In that case we first label the internal nodes of the
tree.
#+end_export
...
#+end_src
#+begin_export latex
Now we can find our target clade, 29, with the program
\ty{fintac}. Its output tells us that clade 29 has a perfect ``split''
in the sense that it contains only O111:H8. It also contains no
``unknowns'', which we will learn more about in the next chapter. We
also note that the distance to its parent is $6\times 10^{-3}$
substitutions per site. This isn't a lot, but Figure~\ref{fig:o111}
shows that this branch clearly separates O111:H8 from the other
\emph{E. coli} strains.
#+end_export
#+begin_src sh <<Analyze phylogeny, Ch. \ref{ch:p}>>=
...
neighbor genomes. We first construct the directory of target genomes,
\ty{targets}, and link the target genomes into it.
#+end_export
#+begin_src sh <<pilot.sh>>=
  mkdir targets
  pickle 29 all.nwk |
...
#+end_src
#+begin_export latex
Similarly, the directory of neighbor genomes, \ty{neighbors}, holds
the links to the complement of the target genomes.
#+end_export
#+begin_src sh <<pilot.sh>>=
  mkdir neighbors
  pickle -c 29 all.nwk |
...
#+end_src
#+begin_export latex
We construct the Fur database, \ty{pilot.db}. This takes roughly 14\,s
on our laptop.
#+end_export
#+begin_src sh <<pilot.sh>>=
...
database and clean them by removing long runs of \ty{N}.
#+end_export
#+begin_src sh <<pilot.sh>>=
  fur -d pilot.db |
...
#+end_src
#+begin_export latex
This yields 157\,kb of candidate marker material.
#+end_export
#+begin_src sh <<pilot.sh>>=
cres pilot.fasta
#+end_src
...
the top primer pairs from them. The best of these top primer pairs has
the very low penalty of $1.4\times 10^{-4}$.
#+end_export
#+begin_src sh <<pilot.sh>>=
  fa2prim pilot.fasta |
      primer3_core |
...
0.000142   CACCATAATTCTGGGCTACG  CCGCCATACAATCCAGTTAG  ...
...
\end{verbatim}
Please note that these are \emph{not} primers intended for further
use. For a more promising set of primers, please see
Chapter~\ref{ch:m}.

We end the pilot section of this tutorial by cleaning up our working
directory, while keeping the putative markers we found in
\ty{markers.fasta}.
#+end_export
#+begin_src sh <<pilot.sh>>=
