...
appear in all target genomes that are absent from the neighbors.
...
genome regions~\cite{hau21:fur,vie24:mar}. So we first
construct the directory of target genomes, \ty{targets}.
#+end_export
#+begin_src sh <<markers.sh>>=
  mkdir targets
...
construction of \ty{eco2.nwk}. Primers with sufficient sensitivity and
specificity could then be further tested on a comprehensive DNA
database like \ty{nt} before moving on to \emph{in vitro} testing.
#+end_export
#+begin_export latex
...
#+end_export
#+begin_src sh <<markers.sh>>=
  rm -r all acc.txt eco2.nwk md5sum.txt ncbi_dataset \
