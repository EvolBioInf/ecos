#+begin_export latex
Again, our working directory is \ty{tutorial} inside the \ty{o111h8}
repo. We link the phylogeny of all complete \emph{E. coli} genomes
calculated with \ty{phylonium}.
#+end_export
#+begin_src sh <<query.sh>>=
  ln -s ../data/phylonium.nwk all.nwk
#+end_src
#+begin_export latex
We count the 7,354 taxa on the tree by picking its root, node 1, and
counting the listed leaves.
#+end_export
#+begin_src sh <<query.sh>>=
  pickle 1 all.nwk |
      grep -c -v '^#'
#+end_src
#+begin_export latex
Since bacterial species are defined as having an ANI distance of at
least 95, we check what is the largest distance present in our
tree. We clip the output to the first line only.
#+end_export
#+begin_src sh <<query.sh>>=
  midRoot -p all.nwk |
      head -n 1
#+end_src
#+begin_export latex
We notice that the largest distance, 0.0862, which implies an ANI
below the threshold for bacterial species. We find that one of the
most distantly related genomes is located at the root of the tree, has
parent node of 1.
#+end_export
#+begin_src sh <<query.sh>>=
  fintac -t 562_021307345.1 all.nwk
#+end_src
#+begin_export latex
We link the corresponding distance matrix from our tree to our current
directory.

We can then remove our outlier from the matrix using mattools and we
name the resulting matrix ecoli.nwk.
#+end_export
#+begin_src sh <<query.sh>>=
  ln -s ../data/phylonium.dist all.dist
  mat grep -v "562_021307345.1" all.dist > ecoli.nwk
#+end_src
#+begin_export latex
Since we have modified the matrix it would be necessary to recalculate
the tree. We also supply this modified tree which we link to our
directory.
#+end_export
#+begin_src sh <<query>>=
  ln -s ../data/ecoli.nwk .
#+end_src
#+begin_export latex
We then confirm that we have a fully resolved \textit{E.~coli}
phylogenetic tree that fulfills the ANI species criteria.
#+end_export
#+begin_src sh <<query.sh>>=
  midRoot -p ecoli.nwk |
      head -n 1
#+end_src
#+begin_export latex
We can next determine the taxa with complete genomes that belong to the
taxonomic subtree rooted on 991910. We use the program \ty{neighbors}
for this, which requires the Neighbors database, so we link this.
#+end_export
#+begin_src sh <<query.sh>>=
ln -s ../data/neidb
#+end_src
#+begin_export latex
It turns out there is only one taxon in the subtree rooted on 991910,
991910 itself.
#+end_export
#+begin_src sh <<query.sh>>=
  neighbors -L complete -t 991910 -o -l neidb |
      grep -v '^#' |
      awk '{print $3}' |
      sort |
      uniq 
#+end_src
#+begin_export latex
We count the 22 genomes on the tree classified as O111:H8.
#+end_export
#+begin_src sh <<query.sh>>=
  pickle 1 ecoli.nwk |
      grep -c '^991910_'
#+end_src
#+begin_export latex
We use \ty{fintac} to find the clade with the largest split between
O111:H8 genomes and the rest. This is clade 5041 with 7 targets, 3
non-targets, or \emph{neighbors}, and no unknown genomes. Unknown
genomes are defined as those whose TaxID matches that of a species
rank. In this case, genomes that are labelled as being only
\textit{Escherichia coli} with no further information regarding their
type strain or serotype.
#+end_export
#+begin_src sh <<query.sh>>=
  fintac -t '^991910_' ecoli.nwk
#+end_src
#+begin_export latex
\begin{verbatim}
#Clade  Targets  Neighbors  Split (%)  Parent  Dist(Parent)
5041    7        3          99.75      5032    2.13e-05
\end{verbatim}
#+end_export
#+begin_export latex
We suspect that the missing 15 O111:H8 genomes are close by and climb
the tree to find that there are 38 ancestral nodes of 5041.
#+end_export
#+begin_src sh <<query.sh>>=
  climt 5041 ecoli.nwk
#+end_src
#+begin_export latex
\begin{verbatim}
# Up   Node   Branch Length   Cumulative Branch Length
38     1      0               0.01981767
...
13     5005   0.000146        0.00063167
12     5007   0.000385        0.00024667
11     5008   1.45e-05        0.00023217
10     5018   2.63e-05        0.00020587
9      5019   3.1e-05         0.00017487
8      5020   3.48e-05        0.00014007
7      5021   8.27e-06        0.0001318
6      5023   1.62e-05        0.0001156
5      5025   2.18e-05        9.38e-05
4      5027   2.74e-05        6.64e-05
3      5028   2.01e-05        4.63e-05
2      5029   1.23e-05        3.4e-05
1      5032   1.27e-05        2.13e-05
0      5041   2.13e-05        0
\end{verbatim}
We look for the most recent common ancestor of all O111:H8 genomes on
the tree. For this we scan the branch lengths going from 5041 towards
the root and notice that the branches are very short up until node
5007, where the branch distance suddenly more than doubles. We
hypothesize that this is the most recent common ancestor. If this were
true, we'd find the 22 target genomes in node 5007 and less than 22
target genomes in its immediate child, node 5008. We look at node 5007
first and find that, as hypothesized, it contains our 22 target
genomes.
#+end_export
#+begin_src sh <<query.sh>>=
  pickle 5007 ecoli.nwk |
      grep -c '^991910_'
#+end_src
#+begin_export latex
We also find that 5008 contains only 21 target genomes, which makes
node 5007 our target clade.
#+end_export
#+begin_src sh <<query.sh>>=
  pickle 5008 ecoli.nwk |
      grep -c '^991910_'
#+end_src
#+begin_export latex
To put our target clade in context, we plot the subtree rooted on its
parent, 5005, without the labels of the internal nodes.
Figure~\ref{fig:all} shows this tree with the \emph{E. coli} O111:H8
genomes marked $T$ for \emph{target}, its two closest relatives marked
$N$ for \emph{neighbors}, and the node with the highest split score
originally picked by \ty{fintac} marked $S$.
\begin{figure}
  \begin{center}
    \rput(3.8,2.55){$T$}
    \rput(3.8,0.6){$N$}
    \rput(6.15,9.55){$S$}
    \includegraphics{../query/all}
  \end{center}
  \caption{The 48 complete target genomes of \emph{Escherichia coli}
    O111:H8, $T$, in the context of its closest relatives, the two
    neighbors in clade $N$; $S$ is the clade with the highest split
    score in the full species phylogeny.}\label{fig:all}
\end{figure}
#+end_export
#+begin_src sh <<query.sh>>=
  pickle -t 5005 ecoli.nwk |
      land -r |
      plotTree
#+end_src
#+begin_export latex
We count 48 complete genomes in clade 5007. This means we have
discovered 26 complete genomes of \emph{E. coli} O111:H8 that are
classified differently.
#+end_export
#+begin_src sh <<query.sh>>=
  pickle 5007 ecoli.nwk |
      grep -v '^#' -c
#+end_src
#+begin_export latex
Let's take a closer at the taxa in our target clade. Apart from the 22
O111:H8 genomes, we find 18 \emph{E. coli} (562). We shall look up the
remaining five taxon-IDs in a moment.
#+end_export
#+begin_src sh <<query.sh>>=
  pickle 5007 ecoli.nwk |
    grep -v '^#' |
    tr '_' ' ' |
    awk '{print $1}' |
    sort |
    uniq -c |
    sort -n -r
#+end_src
#+begin_export latex
\begin{verbatim}
     22 991910
     18 562
      4 373045
      1 585396
      1 397453
      1 168927
      1 1055542
\end{verbatim}
The remaining five taxa are all classified as serotype O111, which is
reassuring, albeit not O111:H8.
#+end_export
#+begin_src sh <<query.sh>>=
  for a in 373045 585396 397453 168927 1055542; do
      echo "# $a"
      ants $a neidb |
	  tail -n 2
  done
#+end_src
#+begin_export latex
\begin{verbatim}
# 373045
  1     562      Escherichia coli          species
  0     373045   Escherichia coli O111:NM  no rank
...
\end{verbatim}

This means that \textit{E.coli} O111:NM is the serotype that has most
frequently been mistaken for O111:H8. The full description of the
taxon IDs and their corresponding names is shown in Table
\ref{tbl:O111types}.

\begin{table}[h]
  \begin{center}
    \caption{Composition of the O111:H8 target clade.}
    \label{tbl:O111types}
    \begin{tabular}{rlr}
      \hline
      Taxon ID & Name & Count \\
     \hline
      991910  & \textit{Escherichia coli} O111:H8 & 22\\
      562     & \textit{Escherichia coli} & 18\\
      373045  & \textit{Escherichia coli} O111:NM & 4\\
      168927  & \textit{Escherichia coli} O111:H- & 1\\
      397453  & \textit{Escherichia coli} O111:H2 & 1\\
      585396  & \textit{Escherichia coli} O111:H- str. 11128 & 1\\
      1055542 & \textit{Escherichia coli} O111 str. RM9322 &1 \\
      \hline
    \end{tabular}
  \end{center}
\end{table}
#+end_export
#+begin_export latex
We've reached the end of the query section, so we remove the two files
we have produced.
#+end_export
#+begin_src sh <<query.sh>>=
  rm ecoli.nwk neidb
#+end_src
